...
export function createWindowManager(): WindowManager {
  return {
    createMainWindow(): BrowserWindow {
      ...
      ;(mainWindow.webContents as unknown as { on: (event: string, listener: (...args: unknown[]) => void) => void }).on('console-message', (_event: Electron.Event & { level: number; message: string; line: number; sourceId: string }) => {
        try {
